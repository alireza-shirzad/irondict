...
# irondict-xlarge (n2-standard-16) VM. Idempotent — safe to re-run.
...
echo "--- 4) trim bench args to [22, 28] (c3 can fit 34 (1.4 TB RAM); small+medium already measured on n2-highmem) ---"
run_remote "cd ~/irondict; \
  sed -i 's/args = \[22, 28, 34\]/args = [34]/' iron-key-bench/benches/audit.rs iron-key-bench/benches/client_lookup.rs iron-key-bench/benches/server_lookup.rs; \
  sed -i 's/LOG_CAPS: \[u64; 3\] = \[22, 28, 34\]/LOG_CAPS: [u64; 1] = [34]/' iron-key-bench/benches/server_update_keys.rs; \
  sed -i 's/LOG_CAPS: \[usize; 3\] = \[22, 28, 34\]/LOG_CAPS: [usize; 1] = [34]/' iron-key-bench/benches/server_update_reg.rs; \
  sed -i 's/args         = \[22, 28, 34\],/args         = [34],/' iron-key-bench/benches/setup.rs; \
